...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        async def _list_titled_sessions() -> list[dict]:
            ...
            # listing stays scoped to the caller's own platform/room, so
            # desktop (tui/cli) sessions become resumable from a phone only
            # when the operator explicitly configured allow_admin_from (#41220).
            cross_origin = widen and source.platform != Platform.MATRIX
            sessions = await self._session_db.list_sessions_rich(
                source=None if cross_origin else user_source,
                exclude_sources=["tool"] if cross_origin else None,
                session_key=None if (widen or cross_origin) else session_key,
                limit=10,
            )
